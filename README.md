...
|原则|说明|
|---|---|
|✅ **懒加载函数必须能自我删除**|`Remove-Item function:global:name`|
|✅ **加载后立即执行一次**|`& $name @args` 在 `Remove-Item` 后|
|✅ **脚本必须定义 `global:name`**|否则删除后无命令可用|
|✅ **避免在懒加载函数中递归调用 `& $name`**|会导致死循环|
|✅ **高频命令不懒加载**|如 `r`, `cd`, `ls` 等应立即加载|
...
|场景|解决方案|
|---|---|
|脚本只定义变量，不定义函数|改为立即加载|
|多个命令来自一个脚本|懒加载一个入口函数，内部再分发|
|希望支持 `Get-Command` 提前发现|可注册空函数或使用 `CommandDiscovery`（高级）|
...
就能安全实现懒加载，避免 `rreplace` 类似问题。

这种模式非常适合：

- 构建工具（`build`, `test`）
- 管理脚本（`deploy`, `db-migrate`）
- 小众工具（`pdfmerge`, `imgopt`）

而核心交互命令（如 `r`, `g`, `ll`）建议**直接加载**，保证响应速度。